...
public class DashboardController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        switch (page) {
            ...
            case "view-message":
                ...
                break;
            case "view-admin-booking":
                Account account = (Account) session.getAttribute(Constant.SESSION_ACCOUNT);
                int id = account.getId();
                listBooking = bookinginfoDAO.getBookingByAccount(id);
                session.setAttribute("listBooking", listBooking);
                url = "../views/admin/dashboard/view-admin-booking.jsp";
                break;
            default:
                url = "../views/admin/dashboard/view-bus.jsp";
                break;
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        switch (action) {
            ...
            case "delete-message":
                ...
                url = "../views/admin/dashboard/view-message.jsp";
                break;
            case "delete-admin-booking":
                deleteAdminBooking(request);
                Account account = (Account) session.getAttribute(Constant.SESSION_ACCOUNT);
                int id = account.getId();
                listBooking = bookinginfoDAO.getBookingByAccount(id);
                session.setAttribute("listBooking", listBooking);
    ...
    private void deleteAdminBooking(HttpServletRequest request) {
        busDAO = new BusDAO();
        int id = Integer.parseInt(request.getParameter("id"));
        int quantity = Integer.parseInt(request.getParameter("quantity"));
        String busname = request.getParameter("busname");
        bookingDAO.deleteById(id);
        request.setAttribute("msgd", "Delete admin booking successfully !!");
        int totalQuantityRemaining = busDAO.getTotalQuantityByBusName(busname);
        int quantityAfterDelete = totalQuantityRemaining + quantity;
        busDAO.updateQuantityAfterUpdate(quantityAfterDelete, busname);
    }

    private void deleteAllMessage(HttpServletRequest request) {
        ContactDAO contactDAO = new ContactDAO();
        contactDAO.deleteAllMessage();
        request.setAttribute("msgdm", "Delete all message successfully !!");
    }
}
