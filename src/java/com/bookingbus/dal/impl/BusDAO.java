...
public class BusDAO extends DBContext {
    ...
    public int getTotalQuantityByBusId(int busID) {
        int totalQuantityRemaining = 0;

        try {
            connection = this.getConnection();

            String sql = "SELECT b.quantity FROM Bus b WHERE b.id = ?";
            preparedStatement = connection.prepareStatement(sql);
            ...
            if (resultSet.next()) {
                totalQuantityRemaining = resultSet.getInt(1);
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    public void updateQuantity(int quantityAfterBooking, int busID) {
        try {
            connection = this.getConnection();
            String sql = "UPDATE Bus SET quantity = ? WHERE id = ?";
            preparedStatement = connection.prepareStatement(sql);

            preparedStatement.setInt(1, quantityAfterBooking);
    ...
    public void updateQuantityAfterUpdate(int quantityAfterDelete, String name) {
        try {
            connection = this.getConnection();
            
            String sql = "UPDATE Bus SET quantity = ? WHERE name = ?";
            preparedStatement = connection.prepareStatement(sql);
    ...
    public int getTotalQuantityByBusName(String busname) {
        int totalQuantityRemaining = 0;

        try {
            connection = this.getConnection();

            String sql = "SELECT b.quantity FROM Bus b WHERE b.name = ?";
            preparedStatement = connection.prepareStatement(sql);
